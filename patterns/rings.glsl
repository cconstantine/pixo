...
vec3 color = vec3(0.0);
float speed = 0.5;
#define scale (distance(tan(time*speed),atan(time*speed))+2.0)
//float scale = -distance(tan(time*speed),-tan(time*speed))-2.0;
float size = 0.4;
float rad = 5.0;
								//position
#define c1 circ(vec2(0,-1.0)/scale,vec3(0.4,0.1,0.1))		//0°
#define c2 circ(vec2(-0.8660,-0.5)/scale,vec3(0.3,0.1,0.2))	//60°
#define c3 circ(vec2(-0.8660,0.5)/scale,vec3(0.2,0.1,0.3))	//...
#define c4 circ(vec2(0.0,1.0)/scale,vec3(0.1,0.1,0.4))
#define c5 circ(vec2(0.8660,0.5)/scale,vec3(0.2,0.1,0.3))
#define c6 circ(vec2(0.8660,-0.5)/scale,vec3(0.3,0.1,0.2))
