...
{
   if ((position.y <= pos + (0.21+0.1*sin(time))) && (position.y >= pos - (0.21+0.1*sin(time))))
    color = mixcol(1.0 - abs(pos - position.y) / (0.21+0.1*sin(time)), r, g, b);
}
void main( void ) {
  ...
  float t = time*1.1+(position.y*position.x*+0.5+position.x);
