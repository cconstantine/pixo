...
namespace Pixlib {
  ...
  void LedCluster::render(const Pattern& pattern, const IsoCamera& viewed_from, float brightness)
  {
    render_timer.start();
    led_renderer.render(pattern.get_texture(), viewed_from, brightness, fadecandy->get_data(), num_leds()*3);
    render_timer.end();
